| 功能模块          | 技术栈                                                               | 选择理由                                                         |
|---------------|-------------------------------------------------------------------|--------------------------------------------------------------|
| JWT 认证        | `github.com/golang-jwt/jwt/v5`                                    | 社区官方维护（原 dgrijalva/jwt-go fork），支持 HS256/RS256，Go Modules 友好 |                                                                   |                                                           |
| RBAC 权限       | `github.com/casbin/casbin/v2` + `github.com/casbin/gin-casbin/v2` | 策略热加载、支持 DB/File 适配器、Gin 中间件开箱即用                             |                                                                   |                                                           |
| 全局错误处理        | 自定义中间件 + `github.com/getsentry/sentry-go`                         | 捕获 panic + 业务错误分级处理，Sentry实时告警 必须设置 `gin.Recovery()` 基础防护    |                                                                   |                                                      |
| 统一响应          | 自定义 `Response` 结构体 + 辅助函数                                         | 轻量可控，避免第三方库耦合                                                |
| 结构化日志         | `go.uber.org/zap` + `github.com/gin-contrib/zap`                  | 性能碾压 logrus（10x+），JSON 格式，Gin 中间件集成                          |
| Redis 客户端     | `github.com/redis/go-redis/v9`                                    | 官方维护（原 go-redis/redis 迁移），支持Cluster/Sentinel/Pipeline        |
| PostgreSQL 驱动 | `github.com/jackc/pgx/v5`（直接使用）                                   | 专为 PG 优化，性能 > database/sql + pq，支持连接池/批量操作                   |
| 参数校验          | `github.com/go-playground/validator/v10`                          | Gin 默认集成，自定义校验标签，多语言错误翻译，与 `binding` 标签无缝配合                  |
| 配置管理          | `github.com/spf13/viper` + 环境变量                                   | 支持 YAML/JSON/ENV，热重载（需自定义） 敏感信息通过 K8s Secret / Vault 注入      |

## Users RESTful + PostgreSQL
